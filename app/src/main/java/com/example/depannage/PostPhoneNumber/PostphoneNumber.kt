...
@Composable
fun PostPhoneNumber() {
    PostPhoneNumberElements()
}

@Composable
fun PostPhoneNumberElements() {
    ...
    fun fetchPosts() {
        db.collection("phone Numbers").get().addOnSuccessListener { documents ->
            ...
            for (document in documents) {
                if (document.get("worker").toString() == auth.currentUser?.uid.toString()) {
                    posts.add(
                        Post(
            ...
            Button(
                onClick = {
                    val post = hashMapOf(
                        ...
                        "worker" to auth.currentUser?.uid.toString(),
                        "wilaya" to selectedItemOne,
                        "daira" to selectedItemTwo,
                        "Name" to name
                    )
                    db.collection("phone Numbers").add(post).addOnSuccessListener {
                        fetchPosts()
                    }
                },
                Modifier.padding(12.dp)
            ) {
                Text(text = "Post the number", color = MaterialTheme.colorScheme.scrim)
            }
